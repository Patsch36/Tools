...
def choose_host_or_ip():
    ...
    def navigate_menu(stdscr):
        ...
        while True:
            stdscr.clear()
            stdscr.addstr(0, 0, "Wähle einen Host oder gebe eine eigene IP ein:")
            for idx, host in enumerate(host_list):
                if idx == current_row:
                    stdscr.addstr(idx + 1, 0, f"{idx + 1}. {host} ({hosts[host]['HostName']})", curses.A_REVERSE)
                else:
                    stdscr.addstr(idx + 1, 0, f"{idx + 1}. {host} ({hosts[host]['HostName']})")

            stdscr.addstr(len(host_list) + 2, 0, "Oder gebe eine eigene IP ein:")
...
def ssh_copy(ip, user, folder_path, remote_path, direction):
    ...
    try:
        ...
        if direction == "to":
            print(f"Erfolgreich kopiert: {folder_path} nach {ip}:{remote_path}")
        else:
            print(f"Erfolgreich kopiert: {ip}:{remote_path} nach {folder_path}")
    except subprocess.CalledProcessError as e:
        print(f"Fehler beim Kopieren: {e}")

def main():
    parser = argparse.ArgumentParser(description='SSH Kopie von oder zu einem Server.')
    parser.add_argument('folder', type=str, help='Pfad des Ordners (lokal oder remote)')
    parser.add_argument('remote_path', type=str, help='Pfad auf dem Server (lokal oder remote)')
    parser.add_argument('direction', choices=['to', 'from'], help="Gib 'to' für Upload oder 'from' für Download an")
